...
namespace utils
{
    NTSTATUS LogToFile(const char* logMessage) {
        ...
        RtlInitUnicodeString(&fileName, L"\\??\\C:\\logs.txt");
        InitializeObjectAttributes(&objAttrs, &fileName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);

        status = ZwCreateFile(&fileHandle, FILE_APPEND_DATA | SYNCHRONIZE, &objAttrs, &ioStatus, NULL,
        ...
        size_t logLength = strlen(logMessage);
        status = ZwWriteFile(fileHandle, NULL, NULL, NULL, &ioStatus, (PVOID)logMessage, (ULONG)logLength, NULL, NULL);
        ZwClose(fileHandle);

        return status;
    }

    PVOID FindModuleBaseAndSize(char* driver_name, PULONG pSize)
    {
        NTSTATUS status;
        ULONG bufferSize = 0x10000;
        PVOID buffer = nullptr, moduleBase = nullptr;
        do {
            buffer = ExAllocatePoolWithTag(NonPagedPool, bufferSize, 'modD');
            if (!buffer) {
                LogToFile("Fail to alocar memória para buffer em FindModuleBaseAndSize\n");
                return nullptr;
            }
            status = ZwQuerySystemInformation(SystemModuleInformation, buffer, bufferSize, &bufferSize);
            if (status == STATUS_INFO_LENGTH_MISMATCH) {
                ExFreePoolWithTag(buffer, 'modD');
                buffer = nullptr;
                bufferSize *= 2;
            }
            else {
        ...
        if (!NT_SUCCESS(status)) {
            if (buffer) ExFreePoolWithTag(buffer, 'modD');
            LogToFile("Falha em ZwQuerySystemInformation\n");
            return nullptr;
        }
        PSYSTEM_MODULE_INFORMATION pModInfo = (PSYSTEM_MODULE_INFORMATION)buffer;
        for (ULONG i = 0; i < pModInfo->ModulesCount; i++) {
            PSYSTEM_MODULE_ENTRY pEntry = &pModInfo->Modules[i];
            if (strstr(pEntry->ImageName, driver_name) != NULL) {
                moduleBase = pEntry->Base;
                if (pSize)
                    *pSize = pEntry->Size;
                break;
            }
